...
![](https://github.com/user-attachments/assets/d2f7ca48-cb7c-47be-bb5a-dd9732f34ce5)
## 📂 Структура проекта
...
- **`src`**
  ...
  - **`app`**
    - **`src/main/java/com/example/android_app`**
      - **`ApiConfig.java`**  
        Конфигурация адреса Flask-сервера.
      - **`MainActivity.java`**  
      ...
      - **`AccessHistoryActivity.java`**  
        Просмотр истории доступа.
      - **`AdminMenuActivity.java`**  
        Админ-меню (добавление, удаление и редактирование пользователей).
